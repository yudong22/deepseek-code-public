    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-[1100]" onClick={onClose}>
      <div className="bg-white dark:bg-[#1c1c1e] w-[460px] rounded-xl shadow-xl flex flex-col border border-zinc-200 dark:border-zinc-800 overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="px-5 py-4 border-b border-[#e3e3e3] dark:border-[#2c2c2e] flex justify-between items-center shrink-0">
          <h3 className="text-sm font-semibold text-zinc-800 dark:text-[#f5f5f7] m-0">设置</h3>
        ...
        <div className="p-5 flex flex-col gap-4 overflow-y-auto">
          ...
          {/* 关于与更新 */}
          <div className="flex flex-col gap-1.5 mt-4 border-t border-[#e3e3e3] dark:border-[#2c2c2e] pt-4">
            <label className="text-[10px] font-bold text-[#8e8e93] tracking-wider uppercase mb-1">关于与更新</label>
            <div className="flex flex-col gap-2">
              <div className="flex gap-3 items-center">
                <span className="text-xs text-zinc-800 dark:text-[#f5f5f7]">当前版本: v{appVersion}</span>
                <button
                  type="button"
                  onClick={onCheckUpdates}
                  disabled={isChecking}
                  className="h-7 px-2.5 bg-[#f2f2f7] dark:bg-[#2c2c2e] hover:bg-[#e5e5ea] dark:hover:bg-[#3a3a3c] border border-zinc-200 dark:border-zinc-800 rounded-md text-xs font-semibold text-zinc-700 dark:text-zinc-300 cursor-pointer transition-colors flex items-center justify-center"
                >
                  {isChecking ? "正在检查..." : "检查更新"}
                </button>
              </div>
              {updateStatus && (
                <div className={`text-xs p-2.5 bg-[#f2f2f7] dark:bg-[#2c2c2e] border border-zinc-250 dark:border-zinc-800 rounded-md mt-1 leading-relaxed ${
        ...
        <div className="px-5 py-4 border-t border-[#e3e3e3] dark:border-[#2c2c2e] flex justify-end gap-2 shrink-0">
          ...
          {savedApiKey && (
            <button className="h-8.5 px-4 bg-[#f2f2f7] dark:bg-[#2c2c2e] hover:bg-[#e5e5ea] dark:hover:bg-[#3a3a3c] border border-zinc-200 dark:border-zinc-800 rounded-md text-xs font-semibold text-zinc-700 dark:text-zinc-300 cursor-pointer transition-colors flex items-center justify-center" onClick={onClear}>
              清除 Key
            </button>
          )}
